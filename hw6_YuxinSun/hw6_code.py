...
class MyDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        labs_tensor = torch.LongTensor([13]*5)
...
def train_yolo(net, batch_size=8, epoch_size=10, *, device="cpu"):
        ...
        for iter, data in enumerate(train_data_loader):
            ...
            if iter % 100 == 99:
                ...
                print("\n[epoch %d/%d] [iter %4d] [elapsed time %5d secs]  mean loss: %7.4f"\
                      %(epoch+1, epoch_size, iter+1, elapsed_time, avg_loss))
...
def grid_plot(imgs, row_title=None, **imshow_kwargs):
...
def test_yolo(net, *, device="cpu", plot_image=False):
    ...
    net.eval()
    net = net.to(device)
    test_data_loader = torch.utils.data.DataLoader(net.val_dataset, batch_size=1, shuffle=False)
    num_anchor_box = 5
    yolo_interval = 8
        ...
        for iter, data in enumerate(test_data_loader):
            if iter % 50 == 49:
                ...
                for icx in retained_cells:
                    ...
                    if torch.all(class_labels_prob < 0.2):
                        pred_catId = None
                    else:
                ...
                if plot_image and len(images) < 16:
                    display_scale = 1
                    ...
                    images.append(fig)
    grid_plot(np.reshape(images, (4,4)))
    plt.savefig(Path.cwd()/"test_images.png")
...
def calc_IoU(bbox1, bbox2):
    ...
    b2[2:4]+= b2[0:2]
    return torchvision.ops.distance_box_iou(b1.unsqueeze(dim=0), b2.unsqueeze(dim=0))
...
if __name__ == "__main__":
    ...
    cate = ['bus', 'cat', 'pizza']
    pwd = Path.cwd()
    datainfo = DataInfo(pwd/'coco', type='train2014', categories=cate)
    train_set = MyDataset(datainfo, update=False)
    datainfo = DataInfo(type='val2014', categories=cate)
    val_set = MyDataset(datainfo, update=False)
    net = HW6_YOLO(train_set, val_set, ngf=32, size=256)
    gc.collect()

    try:
        net.load_state_dict(
            torch.load('/home/parry/gitRepos/homeworks_ECE_60146/hw6_YuxinSun/hw6_yolo.pth')
        )
    except:
        pass
    loss_record = train_yolo(net, device=dml)
    plt.plot(loss_record, title='loss')
    plt.savefig(pwd/'hw6_yolo_loss.png')
    torch.save(
        net.state_dict(), 
        pwd/"hw6_yolo.pth"
    )
